---
name: paoding-project-walkthrough
version: "1.6.1"
...
```
/paoding-project-walkthrough [path] [--depth brief|medium|deep|all] [--audience general|dev] [--lang zh|zh-pure|en|bilingual] [--no-confirm] [--version]
```
...
**Examples:**
```
/paoding-project-walkthrough                                              # Brief + general + zh (CWD)
/paoding-project-walkthrough /path/to/project                             # Brief + general + zh
/paoding-project-walkthrough /path/to/project --depth medium              # Medium + general + zh
/paoding-project-walkthrough /path/to/project --depth deep --audience dev # Deep + dev + zh
/paoding-project-walkthrough --depth deep                                 # Deep + general + zh (CWD)
/paoding-project-walkthrough --audience dev                               # Brief + dev + zh (CWD)
/paoding-project-walkthrough --depth all --audience dev                   # All depths + dev + zh (CWD)
/paoding-project-walkthrough ./deep --depth brief                         # Path "./deep", brief + general + zh
/paoding-project-walkthrough /path/to/project --lang en                   # Brief + general + en
/paoding-project-walkthrough --depth deep --lang en                       # Deep + general + en (CWD)
/paoding-project-walkthrough --depth deep --no-confirm                    # Deep + skip confirmation
/paoding-project-walkthrough /path/to/project --lang zh-pure              # Brief + general + zh-pure
/paoding-project-walkthrough /path/to/project --lang bilingual            # Brief + general + bilingual
/paoding-project-walkthrough --version                                    # Print version and exit
```
...
The output directory name MUST use the skill's name prefix: `<prefix>_project_study_<project-name>/`, where `<prefix>` comes from the `name` field in frontmatter (e.g., `ruyi` from `paoding-project-walkthrough` → `ruyi_project_study_my-codebase/`).
...
If the script outputs an upgrade reminder, inform the user: "A new version is available. You can continue with the current version, or run `claude plugin update paoding-skills@paoding-skills` to upgrade."

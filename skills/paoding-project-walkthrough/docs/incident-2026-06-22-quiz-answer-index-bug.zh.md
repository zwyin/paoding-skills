...
**文件：** `skills/paoding-project-walkthrough/scripts/md_to_html.py` L247
...
**文件：** `skills/paoding-project-walkthrough/scripts/md_to_html.py` L253-254
...
**新增文件：** `skills/paoding-project-walkthrough/tests/test_quiz_parsing.py` — 7 个用例：
...
v0.9.2 加的 SessionStart hook（见 §5.2）让 `scripts/check_update.sh` 一个潜在的旧 bug 变得可见。症状：刚跑完 `claude plugin update paoding-skills@paoding-skills`，下一次 SessionStart 立刻弹一条假的"🔄 有新版本可用（新本地 → 旧缓存远程）"，方向反。
...
**brand-sync 机制：** `brand-sync-tool/sync.sh` 从 `paoding-skills` rsync 到每个品牌的 `output/{brand}/`（再推到该品牌的 GitHub 仓）。L96 `RSYNC_EXCLUDES` 不排除 `scripts/`，所以 `scripts/md_to_html.py` 会被传播。L264-273 Step 9a 会对 `scripts/check_update.sh` 做品牌名替换（`REPO=` 和 `PLUGIN_KEY=`）。

**本地验证：** `bash sync.sh --dry-run` 三品牌 11/11 传播成功（paoding `d24de66`、davinci `59140fe`、doraemon `1256a10`，每个都是 `sync: update from paoding-skills @ fb5b066`）。

**推送：** 去掉 `--dry-run` 重跑，网络失败时重试。**paoding `43816c0`、davinci `697a42c`、doraemon `76acef2`** 全部推到 `zwyin/{brand}-skills.git`。每个 `sync: update from paoding-skills @ b204bd4`（之后是 `181ca23`）。

### 5.3 渠道 C：本机 Claude Code 插件

`~/.claude/plugins/marketplaces/paoding-skills` 是 `zwyin/paoding-skills` 的 git clone。用户跑 `/plugin` 重装 → cache 更新到 `b204bd4`（v0.9.2）再到 `181ca23`（v0.9.3）。验证：`installed_plugins.json` 的 `gitCommitSha` = 最新，`hooks/session-start` 每次 SessionStart 运行，输出合法 JSON `additionalContext`。
...
`skills/paoding-project-walkthrough/scripts/release.sh`（为合集前布局设计）算出 `ROOT = scripts/..` 然后相对 `ROOT` 读 `skills/paoding-project-walkthrough/SKILL.md`。在当前 paoding-skills 合集布局下解析成 `skills/paoding-project-walkthrough/skills/paoding-project-walkthrough/SKILL.md`（不存在）。它还卡 `branch == develop`（我们在 main）。
...
4 个仓库（paoding-skills + 3 品牌仓）每轮都在 `main` + git tag `v0.9.X` 上收到：
...
- CHANGELOG：`skills/paoding-project-walkthrough/CHANGELOG.md` 中 `[1.6.2]`、`[1.6.3]`、`[1.6.4]` 三条
- 测试文件（新增）：`skills/paoding-project-walkthrough/tests/test_quiz_parsing.py`
- 同类文档参考：`docs/fix-proposal-empty-html.md`（结构相似：bug → 根因 → 修复策略 → 纵深防御）
- SessionStart hook 机制：`~/.claude/plugins/marketplaces/paoding-skills/skills/paoding-project-walkthrough/hooks/hooks.json` + `hooks/session-start`
- 跨会话 memory：`memory/quiz-bug-regen-followup.md`、`memory/paoding-skills-update-check-hook.md`

...
CURRENT_VERSION=$(grep '^version:' skills/paoding-project-walkthrough/SKILL.md | head -1 | sed 's/version: * "//;s/"//')
if [ -z "$CURRENT_VERSION" ]; then
    echo "ERROR: Cannot read version from skills/paoding-project-walkthrough/SKILL.md frontmatter" >&2
    exit 1
fi
echo "    Current version: $CURRENT_VERSION → $VERSION"

# 1. Update SKILL.md frontmatter
sed -i.bak "s/^version: \"${CURRENT_VERSION}\"/version: \"${VERSION}\"/" skills/paoding-project-walkthrough/SKILL.md
rm -f skills/paoding-project-walkthrough/SKILL.md.bak

# 2. Update SKILL.md --version output text (all version references)
sed -i.bak "s/project-walkthrough v${CURRENT_VERSION}/project-walkthrough v${VERSION}/g" skills/paoding-project-walkthrough/SKILL.md
sed -i.bak "s/Project Walkthrough v${CURRENT_VERSION}/Project Walkthrough v${VERSION}/g" skills/paoding-project-walkthrough/SKILL.md
rm -f skills/paoding-project-walkthrough/SKILL.md.bak
...
git add skills/paoding-project-walkthrough/SKILL.md .claude-plugin/plugin.json .claude-plugin/marketplace.json CHANGELOG.md README.md

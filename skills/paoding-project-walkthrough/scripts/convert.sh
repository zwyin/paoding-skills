...
#
# Reads:  skills/paoding-project-walkthrough/SKILL.md (canonical source)
# Writes: cursor/project-walkthrough.mdc
#         .windsurf/rules/project-walkthrough.md
#         .opencode/skills/paoding-project-walkthrough/SKILL.md

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SKILL="$ROOT/skills/paoding-project-walkthrough/SKILL.md"
...
generate_cursor() {
    local target="$ROOT/cursor/paoding-project-walkthrough.mdc"
...
generate_windsurf() {
    local target="$ROOT/.windsurf/rules/paoding-project-walkthrough.md"
...
generate_opencode() {
    local target="$ROOT/.opencode/skills/paoding-project-walkthrough/SKILL.md"
    mkdir -p "$(dirname "$target")"
...
check_sync() {
    ...
    # Cursor
    local cursor_check
    cursor_check=$(run_python check "$SKILL" "$ROOT/cursor/paoding-project-walkthrough.mdc")
    ...
    opencode_check=$(run_python check "$SKILL" "$ROOT/.opencode/skills/paoding-project-walkthrough/SKILL.md")
    ...
    windsurf_chars=$(wc -c < "$ROOT/.windsurf/rules/paoding-project-walkthrough.md" | tr -d ' ')

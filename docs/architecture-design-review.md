...
- **问题**: 写的是 `github-safe-publish/`、`project-walkthrough/`，实际是 `paoding-github-safe-publish/`、`paoding-project-walkthrough/`
- **修改方案**: 更新为实际目录名
- **风险**: 无 — 纯文档修改
...
- **问题**: 文档说平台文件输出到 gitignored 的 dist/，实际 paoding-project-walkthrough 的平台文件（cursor/.windsurf/.opencode）已 git 跟踪并提交在 skill 目录内；paoding-github-safe-publish 的平台文件在 dist/ 下未被跟踪。两个 skill 策略不一致
